# Parts of the code have been copied from https://github.com/Martlgap/FaceIDLight/
...
class FaceRecognition:
    def __init__(self, models_path, images_path):
        ...
        self._names = []
        self._faces = []
        self._images = images_path

        self._min_face_size = 40
        self._steps_threshold = [0.6, 0.7, 0.7]
        self._scale_factor = 0.7
        
        self.interpreter = Interpreter(models_path + "mobileNet.tflite")
        self.p_net = Interpreter(models_path + "p_net.tflite")
        self.r_net = Interpreter(models_path + "r_net.tflite")
        self.o_net = Interpreter(models_path + "o_net.tflite")
        
        self.load_saved_embs()

    def load_saved_embs(self):
        files = glob.glob("{}/*.jpg".format(self._images)) + glob.glob("{}/*.png".format(self._images))
        for file in files:
            img = cv2.imread(file)
            # TODO check if image is too large!
            detections = self.detect_faces(img)  # Must be BGR and float32 [0..255]
            if not detections:
                continue
            _, points, _ = detections[0]  # Only take highest-score face
            self._names.append(os.path.basename(file).split(".")[0])
            face = self.get_face(img, points)
            self._faces.append(
        ...
        self._embs = self.get_emb(np.asarray(self._faces))[0]
    ...
    def recognize_faces(self, img):
        ...
        embs = self.get_emb(np.asarray(faces))[0]  # RGB float32 [0..1]
        ids = []
        for i in range(embs.shape[0]):
            pred, dist, conf = self.identify(np.expand_dims(embs[i], axis=0), self._embs, thresh=0.6)
            ids.append(
                [
                    self._names[pred] if pred is not None else "Other",
                    cv2.cvtColor(self._faces[pred] * 255, cv2.COLOR_RGB2BGR) if pred is not None else None,
                    dist,
                    conf,
                ]
            )
    ...
    def tflite_inference(self, model, img):
    ...
    def get_emb(self, img):
        ...
        """
        return self.tflite_inference(self.interpreter, img)
    ...
    def __stage1(self, image, scales: list, stage_status: StageStatus):
        ...
        for scale in scales:
            ...
            out = self.tflite_inference(self.p_net, img_y)
    ...
    def __stage2(self, img, total_boxes, stage_status: StageStatus):
        ...
        out = self.tflite_inference(self.r_net, tempimg1)
    ...
    def __stage3(self, img, total_boxes, stage_status: StageStatus):
        ...
        out = self.tflite_inference(self.o_net, tempimg1)
        out0 = np.transpose(out[0])
        out1 = np.transpose(out[1])
    ...
    def predict(self, image_file):
        ...
        predictions = self.recognize_faces(image)
        res = []
